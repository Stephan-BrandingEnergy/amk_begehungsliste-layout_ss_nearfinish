...
@Component({
  selector: 'app-mangel-edit-massnahme',
  templateUrl: './mangel-edit-massnahme.component.html',
  styleUrls: ['./mangel-edit-massnahme.component.scss']
})
export class MangelEditMassnahmeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // lade Massnahmen list
    this._subscription.add(this.apiService.getMassnahmenList(this.auth.currentLogin?.user.uid)
      .subscribe(response => this.massnahmen = response)
    );
  }
